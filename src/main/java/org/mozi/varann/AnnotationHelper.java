...
public class AnnotationHelper {
    ...
    public DBVariantContextAnnotator getAnnotationDriver(String db, String ref) throws JannovarVarDBException {
        switch (db){
            case "1k":
                return new DBVariantContextAnnotatorFactory().constructThousandGenomes(dataLoader.getDbPathMap().get("1k"), referenceRepo.findById(ref).get(), DBAnnotationOptions.createDefaults());
            case "clinvar":
                return new DBVariantContextAnnotatorFactory().constructClinVar(dataLoader.getDbPathMap().get("clinvar"), referenceRepo.findById(ref).get(), DBAnnotationOptions.createDefaults());
            case "dbsnp":
                return new DBVariantContextAnnotatorFactory().constructDBSNP(dataLoader.getDbPathMap().get("dbsnp"), referenceRepo.findById(ref).get(), DBAnnotationOptions.createDefaults());
            case "cosmic":
                return new DBVariantContextAnnotatorFactory().constructCosmic(dataLoader.getDbPathMap().get("cosmic"), referenceRepo.findById(ref).get(), DBAnnotationOptions.createDefaults());
            case "exac":
                return new DBVariantContextAnnotatorFactory().constructExac(dataLoader.getDbPathMap().get("exac"), referenceRepo.findById(ref).get(), DBAnnotationOptions.createDefaults());
            default:
